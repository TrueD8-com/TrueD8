   ...
   const onlineLoginUsers =  getonlineLoginUsers()
    //    .then((allSockets: any) => {
            newActiveOffers.map((newActiveOffer) => {
                redisMethods.hashSetMembers(newActiveOffer.userId.toString())
                    .then((userSockets) => {
                        if (userSockets && Array.isArray(userSockets) && userSockets.length > 0) {
                            ...
                            if (newActiveOffer.curTakenId.toString() === rialObj._id.toString()) {
                                redisMethods.hashGetAll(newActiveOffer.curGivenId.toString())
                                    .then((curGivenObj: any) => {
                                        ...
                                        // userSockets.map((e) => {
                                             if (newActiveOffer.theUsersOffer) {
                                                 onlineLoginUsers.emit('new_wallet_values', [modifiedUserWallet1, modifiedUserWallet2])
                                             }
                                ...
                                redisMethods.hashGetAll(newActiveOffer.curTakenId.toString())
...
export const handleAcceptOfferSockets = ({
    ...
    const onlineLoginUsers = getonlineLoginUsers()
    if (status === 'sell') {
        redisMethods.hashGetAll(offer.curGivenId.toString())
            .then((curGiven: any) => {
        ...
        redisMethods.hashGetAll(offer.curTakenId.toString())
            .then((curTaken: any) => {
    ...
    if (handleAcceptor) {
        redisMethods.hashSetMembers(acceptorId)
            .then((result) => {
                if (result && Array.isArray(result) && result.length > 0) {
                    const modifiedAcceptorWallet1 = {
                        shortName: curGivenObj.ab_name,
                        value: status === 'buy' ? Math.ceil(giveObjInAccWal.value) : roundToSix(giveObjInAccWal.value)
                    }
                    const modifiedAcceptorWallet2 = {
                        shortName: curTakenObj.ab_name,
                        value: status === 'buy' ? roundToSix(takenObjInAccWal.value) : Math.ceil(takenObjInAccWal.value) 
                    }

                    result.forEach((e) => {
                        onlineLoginUsers.emit(`new_wallet_values`, [modifiedAcceptorWallet1, modifiedAcceptorWallet2])
                    })
                }
            })
            .catch((err) => {
    ...
    redisMethods.hashSetMembers(creatorId)
        .then((result) => {
            if (result && Array.isArray(result) && result.length > 0) {
                const modifiedcreatorWallet1 = {
                    shortName: curGivenObj.ab_name,
                    value: status === 'buy' ? Math.ceil(givenObjInCreWal.value) : roundToSix(givenObjInCreWal.value),
                    commitment: status === 'buy' ? Math.ceil(givenObjInCreWal.commitment) : roundToSix(givenObjInCreWal.commitment)
                }
                const modifiedcreatorWallet2 = {
                    shortName: curTakenObj.ab_name,
                    value: status === 'buy' ? roundToSix(takenObjInCreWal.value) : Math.ceil(takenObjInCreWal.value)
                }
                result.forEach((e) => {
                    onlineLoginUsers.emit('new_tx_user', modifiedObj)
                    onlineLoginUsers.emit(`new_wallet_values`, [modifiedcreatorWallet1, modifiedcreatorWallet2])
                })
            }
        })
        .catch((err) => {
...
export const handleBuyandSellSockets = ({
    ...
    const onlineLoginUsers = getonlineLoginUsers()
    redisMethods.hashSetMembers(userId)
        .then((userSockets) => {
            if (userSockets && Array.isArray(userSockets) && userSockets.length > 0) {
                redisMethods.hashGetAll(currencyId)
