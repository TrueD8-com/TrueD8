...
const finalObjectInserter = ({ object, key, param, value, traded, sell, buy }) => {
    if (Object.keys(object).includes(key.toString())) {
        object[`${key.toString()}`][`${param}`] += Number(value)
        return object
    } else {
        object[`${key.toString()}`] = {
            traded: traded && param === 'traded' ? value : traded && param !== 'traded' ? 0 : undefined,
            sell: sell && param === 'sell' ? value : sell && param !== 'sell' ? 0 : undefined,
