...
authRoutes.get('/verifyPhoneCode',
  ...
  tryCatch((req, res, next) => {
    ...
    return VerificationPhoneCode.findOne({ $and: [{ sessionId: sessionId }, { code: phoneCode }] })
      .then((item) => {
      ...
      .then(() => {
        if (userId) {
          return User.findOne({ _id: userId })
            .then((user) => {
              if (user && user._id.toString() === userId) {
                return user.updateOne({ $set: { emailVerificationString: newEmailCode._id } })
                  .then(() => {
                    const data = {
                      email: email
                    }
...
authRoutes.post('/sendPasswordVerificationLink',
  ...
  tryCatch((req, res, next) => {
    const email = req.body.email
    return User.findOne({ $and: [{ 'email.address': email }, { 'email.validated': true }] })
      .then((user) => {
        if (!user) {
          const error = new myError(
          ...
          const rand = process.env.NODE_ENV === 'test' ? 'e39459ee-18b4-4967-aaaa-f22fb26a8beb' : uuid()
          // const rand = uuid4()
          ...
          const hash = crypto.createHmac('sha256', process.env.CRYPTO_SECRET)
            .update(rand)
            .digest('hex')
          const verificationCode = new VerificationCode({ code: hash , name : rand })
          return verificationCode.save()
            .then(() => {
              return user.updateOne({ $set: {  resetPasswordVerificationString: new Types.ObjectId(verificationCode._id) } })
                .then(() => {
...
authRoutes.post('/resetPasswordWithPhone',
  ...
  tryCatch((req, res, next) => {
    ...
    if (password !== passwordConfirm) {
      ...
      return VerificationPhoneCode.findOne({ $and: [{ code: verificationCode }, { sessionId: sessionId }] })
        .then((item) => {
